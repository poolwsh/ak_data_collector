...
def create_dag_s_data_database():
    try:
        ...
        if RECREATE_DB:
            logger.debug(f"Recreating database {con.dag_s_data_db_name}")
        ...
        if not exists:
            ...
            logger.debug(f"Creating database {con.dag_s_data_db_name}")
        ...
        # 创建数据库用户和权限
        logger.debug(f"Creating user {con.dag_s_data_user} and granting privileges")
...
def main():
    ...
    # 获取数据库连接
    logger.debug(f"Connecting to database {con.dag_s_data_db_name} as user {con.dag_s_data_user}")
