...
def get_i_code_name_list(redis_conn: redis.Redis, pg_conn, ttl: int = 60 * 60):
    ...
    try:
        _df = dguf.read_df_from_redis(con.INDEX_A_REALTIME_KEY, redis_conn)
        if _df is not None:
    ...
    for symbol in symbols:
        try:
            ...
            if _df is not None and '代码' in _df.columns and '名称' in _df.columns:
                ...
                dguf.write_df_to_redis(con.INDEX_A_REALTIME_KEY, combined_df, redis_conn, ttl)
                if DEBUG_MODE:
...
def update_tracing_table(ak_func_name: str, period: str, i_code: str, last_td: str):
    conn = None
    try:
        conn = PGEngine.get_conn()
        sql = f"""
        ...
        with conn.cursor() as cursor:
            cursor.execute(sql, (ak_func_name, i_code, period, last_td, hostname))
        conn.commit()
        logger.info(f"Tracing data updated for {i_code} in {TRACING_TABLE_NAME}.")
    except Exception as e:
        logger.error(f"Failed to update tracing table for {i_code}: {e}")
        raise AirflowException(e)
    finally:
        if conn:
            PGEngine.release_conn(conn)
...
def prepare_arg_list(ak_func_name: str, period: str):
    ...
    try:
        ...
        for i_code, i_name in i_code_name_list:
            ...
            arg_list.append((i_code, dguf.format_td8(start_date), DEFAULT_END_DATE))
...
def process_batch_data(ak_func_name, period, combined_df, conn):
    ...
    temp_csv_path = dguf.save_data_to_csv(combined_df, f'{ak_func_name}_{period}')
    if temp_csv_path is None:
        raise AirflowException(f"No CSV file created for {ak_func_name}, skipping database insertion.")

    # 将数据从 CSV 导入数据库
    dguf.insert_data_from_csv(conn, temp_csv_path, f'ak_dg_{ak_func_name}_{period}')

    # 更新追踪数据
    last_td = combined_df['td'].max()
    for i_code in combined_df['i_code'].unique():
        update_tracing_table(ak_func_name, period, i_code, last_td)
    ...
    try:
        ...
        for index, (i_code, start_date, end_date) in enumerate(arg_list):
            try:
                ...
                index_data_df = dguf.get_i_code_data(
...
def retry_failed_indexes(ak_func_name: str, period: str):
    try:
        ...
        failed_indexes = dguf.read_list_from_redis(FAILED_INDEXES_CACHE_PREFIX, task_cache_conn)
        if not failed_indexes:
            logger.info("No failed indexes to retry.")
            return

        failed_indexes = [index.decode('utf-8') for index in failed_indexes]
